-- ============================================================================
-- FIX SEARCH RELEVANCE — require actual text/taxonomy signal when searching
-- ============================================================================
-- The original search_listings WHERE clause used:
--   OR w.final_composite > 0.0
-- This was always true because tier_boost + completeness + freshness never
-- equal zero, so premium/featured listings always appeared even when the
-- search query had zero text or taxonomy overlap with them.
--
-- This migration recreates search_listings with a fixed WHERE that requires
-- a genuine relevance signal (FTS match or taxonomy match) when a text query
-- or taxonomy filters are provided.
-- ============================================================================
...
FROM with_labels w
WHERE
  -- ── Browse mode: no query, no taxonomy filters → show all listings ───
  (
  ...
  -- ── Text search mode: must match FTS or taxonomy (not just tier/freshness) ──
  OR (
    p_query IS NOT NULL AND p_query != ''
    AND (w.raw_fts_rank > 0 OR w.raw_taxonomy_score > 0)
  )

  -- ── Combined: text + taxonomy filters → either signal suffices ────────
  OR (
    p_query IS NOT NULL AND p_query != ''

-- ============================================================================
-- PROFILE COMPLETENESS — scoring functions + batch update
-- ============================================================================
-- Computes a 0–100 score for each listing based on how complete its profile is.
...
-- Call: SELECT update_all_profile_completeness();
CREATE OR REPLACE FUNCTION update_all_profile_completeness()
RETURNS TABLE(practitioners_updated int, centers_updated int) AS $$
DECLARE
  p_count int := 0;
  c_count int := 0;
BEGIN
  UPDATE practitioners p
  SET profile_completeness = compute_practitioner_completeness(p)
  WHERE true;
  GET DIAGNOSTICS p_count = ROW_COUNT;

  UPDATE centers c
  SET profile_completeness = compute_center_completeness(c)
  WHERE true;
